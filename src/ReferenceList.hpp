 ...
 * class ReferenceList
 *
 * Uses QList under the hood to store pointers, on the surface receives and returns nothing but the
 * references.
 *
 * NOTE: Any method that calls its QList<T*> counterpart with parameter T, T must not be const, if
